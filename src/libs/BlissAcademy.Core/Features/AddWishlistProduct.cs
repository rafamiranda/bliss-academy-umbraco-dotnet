...
namespace BlissAcademy.Core.Features
{
    public static class AddWishlistProduct
    {
        ...
        public class Handler : IRequestHandler<CommandRequest, CommandResponse>
        {
            ...
            public Handler(BlissAcademyContext dbContext, IHttpContextAccessor httpContextAccessor)//, IMapper mapper)
            {
            ...
            public async Task<CommandResponse> Handle(CommandRequest request, CancellationToken cancellationToken)
            {
                ...
                var wishList = new WishlistProduct
                {
                ...
                if (userId.HasValue)
                {
                    ...
                    if (!_dbContext.WishlistProducts.Any(w => w.ProductId == request.ProductId && w.UserId == userId))
                    {
